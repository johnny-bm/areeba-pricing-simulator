...
-- This script uses DROP POLICY IF EXISTS to avoid conflicts

-- 1. Disable RLS temporarily to break any recursion
...
DROP POLICY IF EXISTS "Authenticated users can view active simulators" ON public.simulators;
...
DROP POLICY IF EXISTS "Authenticated users can view templates" ON public.pdf_templates;
...
DROP POLICY IF EXISTS "Authenticated users can view template sections" ON public.template_sections;
...
DROP POLICY IF EXISTS "Authenticated users can view generated PDFs" ON public.generated_pdfs;
...
CREATE POLICY "Authenticated users can view generated PDFs" ON public.generated_pdfs
    FOR SELECT USING (auth.role() = 'authenticated');
