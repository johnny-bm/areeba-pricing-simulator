 ...
 */
export function calculateTieredPrice(item, quantity) {
 ...
 */
export function getEffectiveUnitPrice(item, quantity) {
 ...
 */
export function getAverageUnitPrice(item, quantity) {
    if (item.pricingType !== 'tiered' || !item.tiers || item.tiers.length === 0) {
        return item.defaultPrice;
    }
 ...
 */
export function getBestTierForQuantity(item, quantity) {
    if (item.pricingType !== 'tiered' || !item.tiers || item.tiers.length === 0) {
        return null;
    }
    // Find the tier that applies to this quantity
    const applicableTiers = item.tiers.filter(tier => quantity >= tier.minQuantity &&
 ...
 */
export function getConfigBasedQuantity(item, config) {
 ...
 */
export function getQuantitySourceDescription(item) {
    const quantityFields = item.quantitySourceFields || [];
    if (quantityFields.length === 0) {
        return null;
    }
    ...
    const descriptions = quantityFields.map(field => sourceDescriptions[field] || `Dynamic Field (${field})`);
    const multiplier = item.quantityMultiplier || 1;
 ...
 */
export function getTierConfigContext(tier, config) {
