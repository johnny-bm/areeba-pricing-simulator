...
import { GuestContactInfo, GuestFormErrors } from '../types';
...
export function useGuestValidation() {
  ...
  const validateName = useCallback((name: string, field: 'firstName' | 'lastName'): boolean => {
    if (!name || !name.trim()) {
  ...
  const validateCompany = useCallback((company: string): boolean => {
  ...
  const validateContactInfo = useCallback((contactInfo: GuestContactInfo): boolean => {
  ...
  const clearError = useCallback((field: keyof GuestFormErrors) => {
    setErrors(prev => ({ ...prev, [field]: undefined }));
  }, []);
