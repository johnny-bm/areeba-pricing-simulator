...
import { DynamicClientConfigBar } from '../../configuration/components/DynamicClientConfigBar';
...
import { AutoAddConfigPanel } from '../../configuration/components/AutoAddConfigPanel';
import { BackendConnectionError } from '../../../components/BackendConnectionError';
import { ConnectionDiagnostics } from '../../../components/ConnectionDiagnostics';
import { GuestContactFormModal } from '../../guest/components/GuestContactFormModal';
import { UnifiedHeader, PageHeader, Footer } from '../../../components/layout';
...
import { LoadingState, SimulatorLoadingState } from '../../../components/LoadingState';
import { useSimulatorLoading } from './SimulatorLoadingManager';
...
export function PricingSimulator({ isGuestMode = false }: PricingSimulatorProps) {
  ...
  const { setLoading } = useSimulatorLoading();
  ...
  const [pricingTypes, setPricingTypes] = useState<Array<{value: string, label: string}>>([]);
  const [billingCycles, setBillingCycles] = useState<Array<{value: string, label: string}>>([]);
  ...
  // Load simulator ID from simulator type with timeout
  useEffect(() => {
    const loadSimulatorId = async () => {
      ...
      try {
        ...
        const timeoutPromise = new Promise((_, reject) => 
          setTimeout(() => reject(new Error('Simulator loading timeout')), 10000)
        );
        
        const simulatorPromise = SimulatorApi.loadSimulatorBySlug(simulatorType);
        const simulator = await Promise.race([simulatorPromise, timeoutPromise]);
        ...
        // Don't set backend error for simulator loading failures
      } finally {
        setSimulatorLoading(false);
      }
    };
  ...
  // Load initial data with optimized loading and caching
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        await RefreshHandler.handleRefresh(simulatorId);
        
        // Use optimized data loader with caching
        try {
          const { services, categories, configurations, pricingTypes, billingCycles } = await OptimizedDataLoader.loadSimulatorData(simulatorId, {
            useCache: true,
            timeout: 20000 // Increased timeout to 20 seconds
          });
          
          setPricingServices(services);
          setCategories(deduplicateCategories(categories));
          setConfigurations(configurations);
          
          // Set pricing types and billing cycles
          const activePricingTypes = (pricingTypes || [])
            .filter(type => type.is_active)
            .map(type => ({ value: type.value, label: type.name }));
          setPricingTypes(activePricingTypes);
          
          const activeBillingCycles = (billingCycles || [])
            .filter(cycle => cycle.is_active)
            .map(cycle => ({ value: cycle.value, label: cycle.name }));
          setBillingCycles(activeBillingCycles);
        } catch (optimizedError) {
          console.warn('Optimized loader failed, falling back to direct API calls:', optimizedError);
          
          // Fallback to direct API calls - ensure simulatorId is passed
          const [servicesResponse, categoriesResponse, configResponse, pricingTypesResponse, billingCyclesResponse] = await Promise.allSettled([
            api.loadPricingItems(simulatorId),
            api.loadCategories(simulatorId),
            api.loadConfigurations(simulatorId), // Pass simulatorId here too
            api.loadPricingTypes(), // Load pricing types globally
            api.loadPricingCycles() // Load billing cycles globally
          ]);
          
          if (servicesResponse.status === 'fulfilled') {
            setPricingServices(servicesResponse.value || []);
          }
          
          if (categoriesResponse.status === 'fulfilled') {
            setCategories(deduplicateCategories(categoriesResponse.value || []));
          }
          
          if (configResponse.status === 'fulfilled') {
            setConfigurations(configResponse.value || []);
          }
          
          if (pricingTypesResponse.status === 'fulfilled') {
            const activePricingTypes = (pricingTypesResponse.value || [])
              .filter(type => type.is_active)
              .map(type => ({ value: type.value, label: type.name }));
            setPricingTypes(activePricingTypes);
          }
          
          if (billingCyclesResponse.status === 'fulfilled') {
            const activeBillingCycles = (billingCyclesResponse.value || [])
              .filter(cycle => cycle.is_active)
              .map(cycle => ({ value: cycle.value, label: cycle.name }));
            setBillingCycles(activeBillingCycles);
          }
        ...
        // Clear global loading state when everything is ready
        setLoading(false);
  ...
  // Show loading state with better UX - keep loading until everything is ready
  if (isLoading || simulatorLoading || authLoading || !simulatorId) {
    return (
          ...
          <div className="space-y-6 w-96 text-center">
            ...
            <div className="space-y-2">
              <h2 className="text-lg font-semibold text-foreground">
                {!simulatorId ? 'Initializing Simulator...' : simulatorLoading ? 'Loading Simulator...' : authLoading ? 'Authenticating...' : 'Loading Data...'}
              </h2>
                ...
                {!simulatorId 
                  ...
                  : simulatorLoading 
                    ...
                    : authLoading 
                      ...
                      : 'Loading services and pricing data...'
                }
              </p>
            </div>
  ...
  return (
    <>
      <UnifiedHeader
        ...
        onLogout={handleLogout}
        showAdminButton={isAuthenticated && (userRole === 'admin' || userRole === 'owner')}
        onAdminClick={() => setShowAdminInterface(true)}
      />
      ...
      <main className="min-h-screen bg-background">
        <div className="container mx-auto px-6 py-8">
          ...
          description={selectedSimulator?.description || "Configure your pricing parameters and calculate costs for your payment solutions"}
        />
