...
export function SimulatorLanding({ onSelectSimulator, onOpenAdmin, onLogout }: SimulatorLandingProps) {
  ...
  // Load simulators from database with caching
  useEffect(() => {
    const loadSimulators = async () => {
      try {
        setIsLoading(true);
        // Use optimized data loader with caching
        const data = await OptimizedDataLoader.loadSimulators({
          useCache: true,
          timeout: 8000
        });
        setSimulators(data);
              ...
              <>
                ...
                {Array.from({ length: 3 }).map((_, index) => (
                  <Card key={index} style={{
                    background: 'linear-gradient(90deg, hsl(var(--muted)) 25%, hsl(var(--muted) / 0.8) 50%, hsl(var(--muted)) 75%)',
                ...
                <Card 
                  ...
                  onClick={() => {
                    if (simulator.isAvailable) {
                      ...
                      setLoading(true, `Loading ${simulator.title}...`);
                      ...
                      // Keep loading state visible until the simulator actually loads
                      // Don't reset it automatically - let the simulator component handle it
                    }
                  }}
                >
