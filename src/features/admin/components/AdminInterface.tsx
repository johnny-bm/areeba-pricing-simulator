...
import { ArrowLeft, LogOut, Settings, Package, Tags, History, CreditCard, Calculator, Zap, Users, Plus, ChevronLeft, ChevronRight, Pencil, Copy, RefreshCw, Download, UserCheck, ChevronDown, ChevronRight as ChevronRightIcon, BarChart3, FileText, Building, Image, Edit, User2, ChevronUp } from 'lucide-react';
...
const SimpleServiceManager = lazy(() => import('../../simulator/components/SimpleServiceManager').then(m => ({ default: m.SimpleServiceManager })));
const ScenarioHistoryTab = lazy(() => import('../../simulator/components/ScenarioHistoryTab').then(m => ({ default: m.ScenarioHistoryTab })));

// Pricing configuration components
const UnitsConfiguration = lazy(() => import('../../configuration/components/pricing/UnitsConfiguration').then(m => ({ default: m.UnitsConfiguration })));
const PricingTypesConfiguration = lazy(() => import('../../configuration/components/pricing/PricingTypesConfiguration').then(m => ({ default: m.PricingTypesConfiguration })));
const BillingCyclesConfiguration = lazy(() => import('../../configuration/components/pricing/BillingCyclesConfiguration').then(m => ({ default: m.BillingCyclesConfiguration })));
const TieredTemplatesConfiguration = lazy(() => import('../../configuration/components/pricing/TieredTemplatesConfiguration').then(m => ({ default: m.TieredTemplatesConfiguration })));
import { ConfigurationDialog } from '../../../components/dialogs/ConfigurationDialog';
import { ScenarioDialog } from '../../../components/dialogs/ScenarioDialog';
import { GuestSubmissionDetailDialog } from '../../../components/dialogs/GuestSubmissionDetailDialog';
...
import { ClientFieldsManager } from '../../configuration/components/ClientFieldsManager';
import { Skeleton } from '../../../components/ui/skeleton';
import { UserManagement as UserManagementComponent } from './UserManagement';
import { AdminContentSkeleton as AdminContentSkeletonComponent, AdminStatsSkeleton, AdminUsersSkeleton, AdminTableSkeleton, AdminNavigationLoading } from './AdminLoadingStates';
import { adminPrefetch } from '../../../utils/adminPrefetch';
...
import { SimulatorManager } from '../../simulator/components/SimulatorManager';
import { SimulatorInfoPage } from '../../simulator/components/SimulatorInfoPage';
import { SimulatorDashboard } from '../../simulator/components/SimulatorDashboard';
import { ThemeToggle } from '../../../components/ThemeToggle';
const PdfBuilderAdmin = lazy(() => import('../../pdfBuilder/components/PdfBuilderAdmin').then(m => ({ default: m.PdfBuilderAdmin })));
...
export function AdminInterface({ 
  ...
  const getCurrentSimulatorAndTab = () => {
    ...
    if (path.includes('/configuration/pricing/units')) return { simulator: null, section: 'pricing-units' };
    if (path.includes('/configuration/pricing/types')) return { simulator: null, section: 'pricing-types' };
    if (path.includes('/configuration/pricing/billing-cycles')) return { simulator: null, section: 'pricing-billing-cycles' };
    if (path.includes('/configuration/pricing/tiered-templates')) return { simulator: null, section: 'pricing-tiered-templates' };
  ...
  const [currentSimulatorId, setCurrentSimulatorId] = useState<string | null>(null);
  const [theme, setTheme] = useState<string>(localStorage.getItem('theme') || 'system');
  const [isPrefetching, setIsPrefetching] = useState(false);
  const [configurations, setConfigurations] = useState<any[]>(initialConfigurations);
  ...
  // Load simulator ID from slug
  useEffect(() => {
    const loadSimulatorId = async () => {
      if (currentSimulatorSlug && currentSimulatorSlug !== 'pdf-builder') {
        try {
          const simulator = await SimulatorApi.loadSimulatorBySlug(currentSimulatorSlug);
          setCurrentSimulatorId(simulator?.id || null);
        } catch (error) {
          console.error('Failed to load simulator ID:', error);
          setCurrentSimulatorId(null);
        }
      } else {
        setCurrentSimulatorId(null);
      }
  ...
  // Prefetch data when section changes
  useEffect(() => {
    const prefetchData = async () => {
      if (currentSection && !isLoading) {
        setIsPrefetching(true);
        try {
          await adminPrefetch.prefetchForRoute(currentSection);
        } catch (error) {
  ...
  const handleForceRefresh = async () => {
    try {
      // Clear cache for current simulator
  ...
  return (
    <SidebarProvider>
      <div className="flex h-screen w-full" style={{ width: '100%', maxWidth: 'none' }}>
        <Sidebar>
          ...
          <SidebarContent className="p-3">
            ...
            <div className="space-y-1 mt-6">
              ...
              <div className="space-y-1">
                {/* All Simulators Menu Item - Moved to top */}
                <SidebarMenu>
                  <SidebarMenuItem>
                    <SidebarMenuButton
        ...
        {/* Main Content */}
        <SidebarInset className="w-full !w-full !max-w-none">
          <div className="flex flex-1 flex-col h-full w-full max-w-none">
            <UnifiedHeader
              pageType="admin"
            ...
            />
            <div className="flex flex-1 flex-col gap-4 p-4 h-full w-full max-w-none">
              {/* Navigation Loading Overlay */}
              {navigatingTo && <AdminNavigationLoading />}

              {/* Content */}
              <div className="flex-1 w-full h-full max-w-none">
                {/* Show skeleton loading when actually loading */}
                {isLoading && (
                  <AdminContentSkeletonComponent />
                )}
                ...
                {!isLoading && (
                  <div>
                    ...
                    {currentSimulator && currentSection === 'client-fields' && (
                      <ClientFieldsManager
                        ...
                        onUpdateConfigurations={async (updatedConfigurations) => {
                          try {
                            await api.saveConfigurations(updatedConfigurations, currentSimulator);
                    ...
                    {!currentSimulatorSlug && currentSection !== 'simulators' && currentSection !== 'users' && currentSection !== 'scenarios' && currentSection !== 'guest-submissions' && currentSection !== 'history' && currentSection !== 'configurations' && currentSection !== 'categories' && currentSection !== 'services' && currentSection !== 'tags' && currentSection !== 'pricing-units' && currentSection !== 'pricing-types' && currentSection !== 'pricing-billing-cycles' && currentSection !== 'pricing-tiered-templates' && (
