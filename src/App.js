...
import { clientConfigPersistence, selectedItemsPersistence, globalDiscountPersistence, simulatorSelectionPersistence, serviceMappingsPersistence, autoAddConfigPersistence, isDatabasePersistenceAvailable, flushPendingSaves } from './utils/databasePersistence';
import { PRICING_TYPES, DISCOUNT_TYPES, DISCOUNT_APPLICATIONS, CATEGORY_IDS, ROLES, TABLES } from './config/database';
...
function AppContent() {
    ...
    const [guestContactSubmitted, setGuestContactSubmitted] = useState(() => {
        if (!isGuestMode)
            return false;
    ...
    useEffect(() => {
        const loadPersistedData = async () => {
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Persistence load timeout')), 5000));
            try {
                ...
                const [persistedSimulator, persistedConfig, persistedItems, persistedDiscount, persistedDiscountType, persistedDiscountApplication, persistedMappings, persistedAutoAddConfig] = await Promise.all([
                ...
                if (shouldSync) {
                    ...
                    Object.entries(persistedMappings).forEach(([serviceId, mapping]) => {
    ...
    useEffect(() => {
        const initializeApp = async () => {
            try {
                ...
                const healthCheckTimeout = new Promise((_, reject) => setTimeout(() => reject(new Error('Health check timeout')), 20000));
    ...
    const convertToLegacyConfig = useCallback((dynamicConfig) => {
        const legacyConfig = {
            clientName: dynamicConfig.clientName,
            projectName: dynamicConfig.projectName,
            preparedBy: dynamicConfig.preparedBy,
            hasDebitCards: dynamicConfig.configValues.hasDebitCards || false,
            hasCreditCards: dynamicConfig.configValues.hasCreditCards || false,
            debitCards: dynamicConfig.configValues.debitCards || 0,
            creditCards: dynamicConfig.configValues.creditCards || 0,
            monthlyAuthorizations: dynamicConfig.configValues.monthlyAuthorizations || 0,
            monthlySettlements: dynamicConfig.configValues.monthlySettlements || 0,
            monthly3DS: dynamicConfig.configValues.monthly3DS || 0,
            monthlySMS: dynamicConfig.configValues.monthlySMS || 0,
            monthlyNotifications: dynamicConfig.configValues.monthlyNotifications || 0,
            monthlyDeliveries: dynamicConfig.configValues.monthlyDeliveries || 0
        };
    ...
    const getDefaultQuantity = useCallback((item, config) => {
    ...
    const handleUpdateServiceMappings = useCallback((newMappings) => {
        setServiceMappings(newMappings);
        const convertedConfig = {
            autoAddRules: {},
            quantityRules: {}
        };
        Object.entries(newMappings).forEach(([serviceId, mapping]) => {
            if (mapping.autoAdd && mapping.configField) {
                if (!convertedConfig.autoAddRules[mapping.configField]) {
                    convertedConfig.autoAddRules[mapping.configField] = [];
                }
    ...
    const calculateSummary = () => {
        ...
        const oneTimeItems = selectedItems.filter(item => item.item.category === CATEGORY_IDS.SETUP || isOneTimeUnit(item.item.unit));
        const monthlyItems = selectedItems.filter(item => item.item.category !== CATEGORY_IDS.SETUP && !isOneTimeUnit(item.item.unit));
    ...
    const handleUpdatePricingServices = async (newServices) => {
        try {
            ...
            for (const service of newServices) {
                ...
                const categoryExists = categories.some(cat => cat.id === service.category || cat.name === service.category);
                if (!categoryExists) {
                    invalidServices.push({
                        service: service.name,
            ...
            setSelectedItems(prevSelected => prevSelected.filter(selected => newServices.some(service => service.id === selected.item.id)).map(selected => {
    ...
    const handleUpdateCategories = async (newCategories, skipSave = false) => {
        try {
            ...
            setCategories(deduplicatedCategories);
            setSelectedItems(prevSelected => prevSelected.filter(selected => deduplicatedCategories.some(cat => cat.id === selected.item.category)));
        }
        catch (error) {
            if (error.message && (error.message.includes('Duplicate category IDs detected') ||
    ...
    const selectedItemIds = useMemo(() => selectedItems.map(item => item.item.id), [selectedItems]);
    ...
    if (authLoading) {
        return (_jsx("div", { className: "min-h-screen bg-background p-4 flex items-center justify-center", children: _jsxs("div", { className: "text-center max-w-md", children: [_jsx("div", { className: "w-24 h-6 mx-auto mb-4", children: _jsx(WordMarkRed, {}) }), _jsx("p", { className: "text-muted-foreground", children: "Checking authentication..." })] }) }));
    }
    if (backendConnectionError && !isLoading) {
        return (_jsxs(_Fragment, { children: [_jsx(BackendConnectionError, { onRetry: handleRetryConnection }), _jsx(ConnectionDiagnostics, {})] }));
    }
    if (isLoading) {
        return (_jsx("div", { className: "min-h-screen bg-background p-4 flex items-center justify-center", children: _jsxs("div", { className: "text-center max-w-md", children: [_jsx("div", { className: "w-24 h-6 mx-auto mb-4", children: _jsx(WordMarkRed, {}) }), _jsx("p", { className: "text-muted-foreground mb-2", children: "Loading pricing simulator..." }), _jsxs("div", { className: "flex items-center justify-center gap-2", children: [_jsx("div", { className: "w-2 h-2 rounded-full animate-pulse bg-primary" }), _jsx("p", { className: "text-sm text-muted-foreground", children: "Connecting to backend..." })] })] }) }));
    }
    return (_jsxs(Routes, { children: [_jsx(Route, { path: "/login", element: isAuthenticated ? (_jsx(Navigate, { to: "/simulators", replace: true })) : (_jsx(LoginPage, { onLoginSuccess: handleLoginSuccess })) }), _jsx(Route, { path: "/forgot-password", element: isAuthenticated ? (_jsx(Navigate, { to: "/simulators", replace: true })) : (_jsx(ForgotPasswordPage, {})) }), _jsx(Route, { path: "/reset-password", element: _jsx(ResetPasswordPage, {}) }), _jsx(Route, { path: "/signup", element: _jsx(SignupPage, {}) }), _jsx(Route, { path: "/simulators", element: (() => {
                    ...
                    return (_jsx(SimulatorLanding, { onSelectSimulator: (simulatorId) => {
                            setSelectedSimulator(simulatorId);
                        ...
                        }, onOpenAdmin: (userRole === ROLES.ADMIN || userRole === ROLES.OWNER)
                            ? () => navigate('/admin')
                            : undefined, onLogout: handleLogout }));
                })() }), _jsx(Route, { path: "/admin", element: isAuthenticated && (userRole === ROLES.ADMIN || userRole === ROLES.OWNER) ? (_jsx(AdminInterface, { onClose: () => window.history.back(), items: pricingServices, categories: categories, selectedItems: selectedItems, clientConfig: legacyConfig, onUpdateItems: handleUpdatePricingServices, onUpdateCategories: handleUpdateCategories, onLogout: handleLogout, onForceRefresh: handleForceRefresh, adminToken: "", currentUserId: userId || '', currentUserRole: userRole || '' })) : (_jsx(Navigate, { to: "/login", replace: true })) }), _jsx(Route, { path: "/issuing-simulator", element: (() => {
                    ...
                    return !pricingServices || pricingServices.length === 0 ? (_jsx("div", { className: "min-h-screen bg-background p-4 flex items-center justify-center", children: _jsxs("div", { className: "text-center max-w-md", children: [_jsx("div", { className: "w-24 h-6 mx-auto mb-4", children: _jsx(WordMarkRed, {}) }), _jsx("h2", { className: "text-lg mb-2", children: "No Data Available" }), _jsx("p", { className: "text-muted-foreground mb-4", children: "The pricing simulator has no services or categories configured. Please contact your administrator." })] }) })) : (_jsx(_Fragment, { children: _jsxs("div", { className: "min-h-screen bg-background p-4", children: [_jsxs("div", { className: "max-w-7xl mx-auto", children: [_jsxs("div", { className: "flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-4", children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx(Button, { onClick: handleBackToLanding, variant: "ghost", size: "sm", className: "text-muted-foreground hover:text-foreground h-8 w-8 p-0", title: "Back to Simulators", children: _jsx(ArrowLeft, { className: "h-4 w-4" }) }), _jsx("div", { className: "w-24 h-6 flex-shrink-0", children: _jsx(WordMarkRed, {}) }), _jsx("div", { className: "border-l border-border h-6" }), _jsxs("div", { children: [_jsx("h1", { children: "Issuing & Processing Simulator" }), _jsx("p", { className: "text-muted-foreground text-sm", children: "Configure your card payment solution and calculate costs" })] })] }), _jsxs("div", { className: "flex gap-2 justify-end items-center flex-wrap", children: [configurations.length > 0 && (_jsx(AutoAddConfigPanel, { services: pricingServices, clientConfig: clientConfig, configurations: configurations, serviceMappings: serviceMappings, autoAddConfig: autoAddConfig, onUpdateServiceMappings: handleUpdateServiceMappings, onUpdateAutoAddConfig: handleUpdateAutoAddConfig })), _jsxs(Button, { onClick: handleClearScenario, variant: "outline", size: "sm", className: "text-xs sm:text-sm", children: [_jsx("span", { className: "hidden sm:inline", children: "Clear Scenario" }), _jsx("span", { className: "sm:hidden", children: "Clear" })] }), _jsx("div", { className: "border-l border-border h-8" }), _jsx(UserProfileHeader, { onLogout: handleLogout })] })] }), _jsx(DynamicClientConfigBar, { config: clientConfig, onConfigChange: setClientConfig, isGuestMode: isGuestMode }), _jsxs("div", { className: "grid grid-cols-1 lg:grid-cols-3 gap-4", children: [_jsx("div", { className: "lg:col-span-1 order-1", children: _jsx(ItemLibrary, { items: pricingServices, categories: categories, selectedItemIds: selectedItemIds, selectedItems: selectedItems, onAddItem: handleAddItem, onRemoveItem: handleRemoveItem, clientConfig: legacyConfig }) }), _jsx("div", { className: "lg:col-span-1 order-2", children: _jsx(ScenarioBuilder, { selectedItems: selectedItems, onUpdateItem: handleUpdateItem, onRemoveItem: handleRemoveItem, clientConfig: legacyConfig, categories: categories, isGuestMode: isGuestMode }) }), _jsx("div", { className: "lg:col-span-1 lg:sticky lg:top-4 lg:self-start order-3 lg:order-3 lg:max-h-[calc(100vh-2rem)] lg:overflow-auto", children: _jsx(FeeSummary, { selectedItems: selectedItems, categories: categories, globalDiscount: globalDiscount, globalDiscountType: globalDiscountType, globalDiscountApplication: globalDiscountApplication, onGlobalDiscountChange: setGlobalDiscount, onGlobalDiscountTypeChange: setGlobalDiscountType, onGlobalDiscountApplicationChange: setGlobalDiscountApplication, clientConfig: clientConfig, onSubmit: handleSubmitScenario, isSubmitting: isSubmitting, isGuestMode: isGuestMode, guestContactSubmitted: guestContactSubmitted, onShowGuestContactForm: () => setShowGuestContactForm(true) }) })] }), _jsx("footer", { className: "mt-12 pt-8 border-t border-border", children: _jsxs("div", { className: "flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 text-sm text-muted-foreground", children: [_jsxs("div", { className: "flex items-center gap-2", children: ["areeba \u00A9 ", new Date().getFullYear(), ". All Rights Reserved.", _jsx(VersionInfo, { simple: true })] }), _jsx("div", { className: "flex items-center gap-4", children: _jsx("a", { href: EXTERNAL_URLS.AREEBA_PRIVACY, target: "_blank", rel: "noopener noreferrer", className: "hover:text-foreground transition-colors", children: "Privacy" }) })] }) })] }), _jsx(ScenarioSummaryDialog, { open: showSummaryDialog, onOpenChange: setShowSummaryDialog, scenarioId: savedScenarioId, clientConfig: clientConfig, summary: calculateSummary(), selectedItems: selectedItems, categories: categories, onDownloadPDF: handleDownloadPDF }), _jsx(GuestContactFormModal, { isOpen: isGuestMode && showGuestContactForm, onClose: () => setShowGuestContactForm(false), onSubmit: handleGuestContactSubmit }), _jsx(Toaster, {})] }) }));
                })() }), _jsx(Route, { path: "/", element: isAuthenticated ? (_jsx(Navigate, { to: "/simulators", replace: true })) : (_jsx(Navigate, { to: "/login", replace: true })) }), _jsx(Route, { path: "*", element: _jsx(Navigate, { to: "/login", replace: true }) })] }));
}
const ErrorFallback = ({ error }) => (_jsx("div", { className: "min-h-screen bg-background p-4 flex items-center justify-center", children: _jsxs("div", { className: "text-center max-w-md", children: [_jsx("div", { className: "w-16 h-16 mx-auto mb-4 flex items-center justify-center bg-orange-100 dark:bg-orange-950/20 rounded-full", children: _jsx(FileText, { className: "h-8 w-8 text-orange-600" }) }), _jsx("h2", { className: "text-xl mb-2", children: "Application Error" }), _jsx("p", { className: "text-muted-foreground mb-4", children: "Something went wrong. Please refresh the page to continue." }), _jsx("div", { className: "flex gap-2 justify-center mb-4", children: _jsx(Button, { onClick: () => window.location.reload(), variant: "default", children: "Refresh Page" }) }), _jsx("div", { className: "p-3 bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-800 rounded-lg", children: _jsxs("p", { className: "text-sm text-orange-800 dark:text-orange-200", children: [_jsx("strong", { children: "Error:" }), " ", error.message] }) })] }) }));
export default function App() {
    return (_jsx(ErrorBoundary, { fallback: ErrorFallback, children: _jsx(BrowserRouter, { children: _jsx(AppContent, {}) }) }));
}
