-- PDF Builder System Database Schema (Safe Version)
-- This version handles existing objects gracefully

-- Enable necessary extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create enum types for section types (only if they don't exist)
DO $$ 
BEGIN
...
CREATE INDEX IF NOT EXISTS idx_content_sections_created_at ON content_sections(created_at);

CREATE INDEX IF NOT EXISTS idx_pdf_templates_simulator_type ON pdf_templates(simulator_type);
...
CREATE INDEX IF NOT EXISTS idx_template_sections_section_id ON template_sections(section_id);
CREATE INDEX IF NOT EXISTS idx_template_sections_position ON template_sections(template_id, position);

CREATE INDEX IF NOT EXISTS idx_generated_pdfs_template_id ON generated_pdfs(template_id);
CREATE INDEX IF NOT EXISTS idx_generated_pdfs_simulator_type ON generated_pdfs(simulator_type);
CREATE INDEX IF NOT EXISTS idx_generated_pdfs_generated_by ON generated_pdfs(generated_by);
CREATE INDEX IF NOT EXISTS idx_generated_pdfs_generated_at ON generated_pdfs(generated_at);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_pdf_templates_updated_at') THEN
...
-- Helper function to get user role (only if it doesn't exist)
...
-- Helper function to check if user is admin (only if it doesn't exist)
...
-- Content sections policies (drop and recreate to avoid conflicts)
DROP POLICY IF EXISTS "Users can view all content sections" ON content_sections;
...
DROP POLICY IF EXISTS "Admins can delete content sections" ON content_sections;
...
DROP POLICY IF EXISTS "Admins can insert pdf templates" ON pdf_templates;
DROP POLICY IF EXISTS "Admins can update pdf templates" ON pdf_templates;
DROP POLICY IF EXISTS "Admins can delete pdf templates" ON pdf_templates;
...
DROP POLICY IF EXISTS "Users can view all template sections" ON template_sections;
DROP POLICY IF EXISTS "Admins can insert template sections" ON template_sections;
DROP POLICY IF EXISTS "Admins can update template sections" ON template_sections;
DROP POLICY IF EXISTS "Admins can delete template sections" ON template_sections;
...
DROP POLICY IF EXISTS "Users can insert generated pdfs" ON generated_pdfs;
DROP POLICY IF EXISTS "Admins can update generated pdfs" ON generated_pdfs;
DROP POLICY IF EXISTS "Admins can delete generated pdfs" ON generated_pdfs;
...
-- Create Supabase Storage bucket for PDF section images (only if it doesn't exist)
INSERT INTO storage.buckets (id, name, public)
VALUES ('pdf-section-images', 'pdf-section-images', true)
ON CONFLICT (id) DO NOTHING;

-- Storage policies for PDF section images (drop and recreate to avoid conflicts)
DROP POLICY IF EXISTS "Public can view PDF section images" ON storage.objects;
DROP POLICY IF EXISTS "Authenticated users can upload PDF section images" ON storage.objects;
DROP POLICY IF EXISTS "Admins can update PDF section images" ON storage.objects;
DROP POLICY IF EXISTS "Admins can delete PDF section images" ON storage.objects;
...
-- Create function to automatically set version numbers for templates (only if it doesn't exist)
...
-- Add trigger for automatic version numbering (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_template_version_trigger') THEN
...
-- Create function to ensure only one active template per simulator type (only if it doesn't exist)
...
-- Add trigger to ensure only one active template per simulator type (only if it doesn't exist)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'ensure_single_active_template_trigger') THEN
...
-- Create view for template sections with section details (only if it doesn't exist)
CREATE OR REPLACE VIEW template_sections_with_details AS
SELECT 
  ts.id,
  ts.template_id,
  ts.section_id,
...
-- Create view for active templates with section counts (only if it doesn't exist)
CREATE OR REPLACE VIEW active_templates_with_sections AS
SELECT 
  pt.id,
  pt.template_name,
  pt.simulator_type,
...
-- Create function to duplicate a template (only if it doesn't exist)
...
-- Create function to get template hierarchy for a simulator type (only if it doesn't exist)
